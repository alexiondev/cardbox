...
impl From<Vec<u8>> for Sav1 {
    fn from(data: Vec<u8>) -> Self {
        ...
        let party_data =
            &sav.data[SAV1_PARTY_DATA_OFFSET..SAV1_PARTY_DATA_OFFSET + SAV1_PARTY_DATA_LENGTH];
        for i in 0..sav.party.len() {
            sav.party[i] = Pk1::from_party_data(party_data, i);
        }

        let curr_box_data =
            &sav.data[SAV1_CURR_BOX_DATA_OFFSET..SAV1_CURR_BOX_DATA_OFFSET + SAV1_BOX_DATA_LENGTH];
